...
function Spawn_NPC( ply, NPCClassName, WeaponName, tr )
	...
	if ( list.Get( "NPC" )[NPCClassName].AdminOnly && !ply:IsAdmin() ) then
	if admino.InternalServerGet( NPCClassName, ply ) ~= false then
	   return
	   end
	end
	
	if admino.InternalServerGet( NPCClassName, ply ) == true then
	if ply:IsAdmin() == false and ply:IsSuperAdmin() == false then
	   return
	   end
	end
...
local function CanPlayerSpawnSENT( ply, EntityName )
	...
	if ( sent == nil ) then
		...
		if ( EntTable.AdminOnly && !ply:IsAdmin() ) then
		if admino.InternalServerGet( EntityName, ply ) ~= false then
		return false
		   end
		end
		
		if admino.InternalServerGet( EntityName, ply ) == true then
		if ply:IsAdmin() == false and ply:IsSuperAdmin() == false then
		   return false 
		   end
		end
	...
	if ( scripted_ents.GetMember( EntityName, "AdminOnly" ) && !ply:IsAdmin() ) then
	if admino.InternalServerGet( EntityName, ply ) ~= false then
	   return false
	   end
	end
...
function CCGiveSWEP( ply, command, arguments )
	...
	if admino.InternalServerGet( swep.ClassName, ply ) ~= false then
	   return
	   end
	end
	
	if admino.InternalServerGet( swep.ClassName, ply ) == true then
	if ply:IsAdmin() == false and ply:IsSuperAdmin() == false then
	   return
	   end
	end
...
function Spawn_Weapon( ply, wepname, tr )
	...
	-- You're not allowed to spawn this!
	if ( ( !swep.Spawnable && !ply:IsAdmin() ) or ( swep.AdminOnly && !ply:IsAdmin() ) ) then
	if admino.InternalServerGet( wepname, ply ) ~= false then
	   return
	   end
	end
	
	if admino.InternalServerGet( wepname, ply ) == true then
	if ply:IsAdmin() == false and ply:IsSuperAdmin() == false then
	   return
	   end
	end
...
local function MakeVehicle( ply, Pos, Ang, model, Class, VName, VTable, data )
	...
	if ( VTable.AdminOnly && !ply:IsAdmin() ) then
	if admino.InternalServerGet( VName, ply ) ~= false then
	   return
	   end
	end
	
	if admino.InternalServerGet( VName, ply ) == true then
	if ply:IsAdmin() == false and ply:IsSuperAdmin() == false then
	   return
	   end
	end
